фронтенд при ошибках не должен отправлять повторный синхронизирующий запрос, потому что это _может_ приводить к бесконечным зацикливаниям

ограничение: на одну сессию может быть только одна управляемая сущность в одном домене

все роуты в приложении должны заканчиваться
пустой строкой ''
вместо слэша '/'
...
!!если клиент во время ожидания ack`ов получает клиентский бродкаст, то этот же клиент сразу же отправляет запрос на актуальное состояние сущности сервера

    /**
      * пока на сервер отсылалось сообщение о клиентском дрейне,
...
то-есть из dto нужно создать модель, а потом сконвертировать её в запись
...
список открытых сессий (ip адрес, устройство)
